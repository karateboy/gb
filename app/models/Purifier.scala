...
case class Purifier(_id: String, name: String, addr: String, industry: String,
                    location: Option[Seq[Double]] = None,
                    quantity: Option[Double] = None, moisture: Option[Double] = None,
                    sales: Option[String] = None, lastUpdate: Option[Date] = None)

case class QueryPurifierParam(name: Option[String], addr: Option[String])
...
object Purifier {
  ...
  val codecRegistry = fromRegistries(fromProviders(classOf[Purifier]), DEFAULT_CODEC_REGISTRY)

  val ColName = "purifier"
  val collection = MongoDB.database.getCollection[Purifier](ColName).withCodecRegistry(codecRegistry)
    ...
    if (!colNames.contains(ColName)) {
      ...
      f.onSuccess({
        case x =>
          ...
          endF.onComplete({
            case x =>
              val path = current.path.getAbsolutePath + "/import/purifier.xlsx"
  ...
  def parser(sheet: XSSFSheet) {
    var rowN = 1
    var finish = false
    var seq = IndexedSeq.empty[Purifier]
    do {
      ...
      if (row == null)
        ...
        try {
          ...
          val industry = row.getCell(4).getStringCellValue
  ...
  def getFilter(param: QueryPurifierParam) = {
  ...
  def query(param: QueryPurifierParam)(skip: Int, limit: Int) = {
    import org.mongodb.scala.model.Filters._
  ...
  def queryCount(param: QueryPurifierParam) = {
    val filter = getFilter(param)
  ...
  def upsert(_id: String, purifier: Purifier) = {
    val f = collection.replaceOne(Filters.eq("_id", _id), purifier, UpdateOptions().upsert(true)).toFuture()
    f.onFailure {
      errorHandler
    }
    f
