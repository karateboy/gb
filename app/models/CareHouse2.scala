...
case class CareHouse2(_id: String)
                     
case class QueryCareHouse2Param(county: Option[String], name: Option[String], addr: Option[String],
  ...
  def getFilter = {
    ...
    val filterList = List(countyFilter, nameFilter, hasLocationFilter).flatMap { f => f }
...
object CareHouse2 {
  ...
  val codecRegistry = fromRegistries(fromProviders(classOf[CareHouse2], classOf[CareType]), DEFAULT_CODEC_REGISTRY)

  val ColName = "careHouse2"
