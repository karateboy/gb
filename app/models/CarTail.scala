...
case class CarTail(_id: String, company: String, weight: Double, var loc: GeoPoint, freeFrom: Long) {
...
object CarTail {
  ...
  implicit val truckRead = Json.reads[CarTail]
  implicit val truckWrite = Json.writes[CarTail]

  def toCarTail(implicit doc: Document) = {
    ...
    CarTail(_id = _id, company = company, weight = weight, loc = loc, freeFrom = freeFrom)
  }
